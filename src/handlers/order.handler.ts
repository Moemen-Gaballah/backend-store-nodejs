...
export const store = async (req: Request, res: Response) => {
  try {
    ...
    if (
      ...
      res.json(
        apiResponse(
          ...
          "All field (:products, :status, :user_id) is required!"
        )
      );
    ...
    const order: Order = await orderModelInstance.store({
      products,
      status,
      user_id,
    });
    ...
    if (
      ...
      res.json(
        apiResponse(
          "",
          HttpStatusCode.BAD_REQUEST,
          "All field ( :products, :status, :user_id, :id) is required!"
        )
      );
      return false;
    }

    const order: Order = await orderModelInstance.update(id, {
      products,
      status,
      user_id,
    });
