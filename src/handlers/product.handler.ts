...
export const store = async (req: Request, res: Response) => {
    try {
        ...
        if (name === undefined || price === undefined) {
            ...
            res.json(apiResponse("", HttpStatusCode.BAD_REQUEST, "All field ( :name, :price) is required"))
...
export const update = async (req: Request, res: Response) => {
    try {
        ...
        if (name === undefined || price === undefined || id === undefined) {
            res.status(HttpStatusCode.BAD_REQUEST)
            res.json(apiResponse("", HttpStatusCode.BAD_REQUEST, "All field ( :name, :price, :id) is required!"))
            return false;
        }

        const product: Product = await productModelInstance.update(id, {name, price})
...
export const destroy = async (req: Request, res: Response) => {
    try {
        ...
        res.json(apiResponse("", HttpStatusCode.OK, "Product deleted successfully."))
    } catch (e) {
        res.status(HttpStatusCode.BAD_REQUEST)
        res.json(e)
    }
